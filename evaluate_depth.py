...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        depth_decoder = networks.DepthDecoder(encoder.num_ch_enc, scales=[0])
        ...
        if opt.use_inr_filter:
            ...
            with open(opt.config, mode="r") as f:
                ...
                print("Config loaded.")
            filter_model = models_.liif.LIIF(**config["model"]["args"])
    ...
    # 打开 CSV 文件进行写入
    csv_filename = "evalresults/{}.csv".format(
        opt.model_name
    )  # 你可以根据需要修改文件名
    file_exist = os.path.isfile(csv_filename)
    if not file_exist:  # touch a new csv file
