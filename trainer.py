...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.use_inr_filter:
            ...
            if self.config["loadckpt"]:
                ...
                assert (
                    model_spec["name"] == self.config["model"]["name"]
                ), "model name dismatch with ckpt"
                assert (
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in pbar:
            ...
            pbar.set_postfix({"loss": losses["loss"].cpu().data})
    ...
    def process_batch(self, inputs: Dict[Tuple | str, torch.Tensor]):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs.update(self.models["depth"](features))  # add key: ("disp", scale)
        ...
        if self.use_pose_net:
            # add key:
            # ("axisangle", scale, 1 or -1)
            # ("translation", scale, 1 or -1)
            # ("cam_T_cam", scale, 1 or -1)
        ...
        # ("sample", 1 or -1, scale)
        # ("color", 1 or -1, scale)
        # ("color_identity", 1 or -1, scale)
        self.generate_images_pred(inputs, outputs)

        if self.opt.use_inr_filter:
            # add key: "disp_filter"
            resolution, start_point = query.make_qp_params_random(
                self.opt.batch_size,
                (self.opt.height, self.opt.width),
                self.config["query"]["qpsize"],
                (
                    self.config["query"]["resmin"],
                    self.config["query"]["resmax"],
                ),
            )
            query_points = query.make_query_points(
                self.opt.batch_size,
                (self.opt.height, self.opt.width),
                self.config["query"]["qpsize"],
                resolution,
            ...
            # query_points = (
            #     make_coord((self.opt.height, self.opt.width))
            #     .cuda()
            #     .view(-1, 2)
            #     .repeat((self.opt.batch_size, 1, 1))
            # )
            query_grids = query_points / torch.tensor(
            ...
            _, disp_depth = disp_to_depth(
                outputs["disp_filter"], self.opt.min_depth, self.opt.max_depth
            )
            for _, frame_id in enumerate(self.opt.frame_ids[1:]):
                T = outputs[("cam_T_cam", 0, frame_id)]
                outputs.update(
                    {
                        ("warp_rgb_filter", frame_id): query.warp_query_rgb(
                            ...
                            inputs[("inv_K", 0)],
                            T,
                            qpsize=self.config["query"]["qpsize"],
                        )
                    }
                )
    ...
    def predict_poses(self, inputs: Dict[Tuple | str, torch.Tensor], features):
        ...
        if self.num_pose_frames == 2:
            ...
            if self.opt.pose_model_type == "shared":
                ...
                pose_feats: Dict[int | str, torch.Tensor] = {
